...
export const fetch = createAsyncThunk('deal_types/fetch', async (data: any) => {
  const { id, query } = data;
  const result = await axios.get(`deal_types${query || (id ? `/${id}` : '')}`);
  return id
    ? result.data
...
export const deleteItem = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      await axios.delete(`deal_types/${id}`);
    } catch (error) {
      if (!error.response) {
        throw error;
      }
...
export const create = createAsyncThunk(
  ...
  async (data: any, { rejectWithValue }) => {
    try {
      const result = await axios.post('deal_types', { data });
      return result.data;
    } catch (error) {
      if (!error.response) {
        throw error;
...
export const update = createAsyncThunk(
  ...
  async (payload: any, { rejectWithValue }) => {
    try {
      const result = await axios.put(`deal_types/${payload.id}`, {
        id: payload.id,
        data: payload.data,
      });
      return result.data;
