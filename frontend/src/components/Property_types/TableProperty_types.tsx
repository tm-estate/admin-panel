...
import {
  fetch,
  deleteItem,
} from '../../stores/property_types/property_typesSlice';
...
const TableSampleProperty_types = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  useEffect(() => {
    if (property_typesNotify.showNotification) {
      notify(
        property_typesNotify.typeNotification,
        property_typesNotify.textNotification,
      );
    }
          ...
          <tbody>
            {property_types &&
              ...
              property_types.map((item: any) => (
                <tr
                  key={item.id}
                  onClick={() => router.push(`/property_types/${item.id}`)}
                >
        ...
        <div className='flex flex-col md:flex-row items-center justify-between py-3 md:py-0'>
          <div className='flex items-center mr-4'>
            <p className={'text-[#8491A0]'}>
              Displaying{' '}
              {property_types.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
