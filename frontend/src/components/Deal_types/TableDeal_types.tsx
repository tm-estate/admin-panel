...
const TableSampleDeal_types = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (deal_typesNotify.showNotification) {
      notify(
        deal_typesNotify.typeNotification,
        deal_typesNotify.textNotification,
      );
    }
  ...
  console.log('deal_types: ', deal_types);
          ...
          <tbody>
            {deal_types &&
              ...
              deal_types.map((item: any) => (
                <tr
                  key={item.id}
                  onClick={() => router.push(`/deal_types/${item.id}`)}
                >
        ...
        <div className='flex flex-col md:flex-row items-center justify-between py-3 md:py-0'>
          <div className='flex items-center mr-4'>
            <p className={'text-[#8491A0]'}>
              Displaying{' '}
              {deal_types.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
