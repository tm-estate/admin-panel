...
import { fetch, deleteItem } from '../../stores/city_areas/city_areasSlice'
...
const TableSampleCity_areas = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (city_areasNotify.showNotification) {
      notify(city_areasNotify.typeNotification, city_areasNotify.textNotification)
    }
  }, [city_areasNotify.showNotification])
    ...
    <>
      ...
      <div className="relative overflow-x-auto">
        <table>
          ...
          <tbody>
            {city_areas &&
              Array.isArray(city_areas) &&
              city_areas.map((item: any) => (
                <tr key={item._id} onClick={() => router.push(`/city_areas/${item._id}`)}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {city_areas.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {city_areas.length < perPage
                ? city_areas?.length + perPage * currentPage
...
export default TableSampleCity_areas
