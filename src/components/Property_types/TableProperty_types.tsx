...
const TableSampleProperty_types = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (property_typesNotify.showNotification) {
      notify(property_typesNotify.typeNotification, property_typesNotify.textNotification)
    }
  }, [property_typesNotify.showNotification])
        ...
        <table>
          ...
          <tbody>
            {property_types &&
              ...
              property_types.map((item: any) => (
                <tr key={item._id} onClick={() => router.push(`/property_types/${item._id}`)}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {property_types.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {property_types.length < perPage
                ? property_types?.length + perPage * currentPage
...
export default TableSampleProperty_types
