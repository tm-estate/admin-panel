...
import { fetch, deleteItem } from '../../stores/product_parameters/product_parametersSlice'
...
const TableSampleProduct_parameters = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (product_parametersNotify.showNotification) {
      notify(product_parametersNotify.typeNotification, product_parametersNotify.textNotification)
    }
  }, [product_parametersNotify.showNotification])
  ...
  console.log('product_parameters: ', product_parameters)
    ...
    <>
      ...
      <div className="relative overflow-x-auto">
        <table>
          ...
          <tbody>
            {product_parameters &&
              Array.isArray(product_parameters) &&
              product_parameters.map((item: any) => (
                <tr key={item._id} onClick={() => router.push(`/product_parameters/${item._id}`)}>
                  ...
                  <td data-label="selectType">{item.selectType}</td>

                  <td data-label="isRequired">{dataFormatter.booleanFormatter(item.isRequired)}</td>

                  <td data-label="dealTypes">
                    {dataFormatter
                      .deal_typesManyListFormatter(item.dealTypes, 'titleRu')
                      .join(', ')}
                  </td>

                  <td data-label="propertyTypes">
                    {dataFormatter
                      .property_typesManyListFormatter(item.propertyTypes, 'titleRu')
                      .join(', ')}
                  </td>
                  ...
                  <td data-label="items">
                    {dataFormatter
                      .product_parameter_itemsManyListFormatter(item.items, 'titleRu')
                      .join(', ')}
                  </td>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {product_parameters.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {product_parameters.length < perPage
                ? product_parameters?.length + perPage * currentPage
...
export default TableSampleProduct_parameters
