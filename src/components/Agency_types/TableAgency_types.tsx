...
const TableSampleAgency_types = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (agency_typesNotify.showNotification) {
      notify(agency_typesNotify.typeNotification, agency_typesNotify.textNotification)
    }
  }, [agency_typesNotify.showNotification])
    ...
    <>
      ...
      <div className="relative overflow-x-auto">
        <table>
          ...
          <tbody>
            {agency_types &&
              Array.isArray(agency_types) &&
                agency_types.map((item: any) => (
                <tr key={item._id} onClick={() => router.push(`/agency_types/${item._id}`)}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {agency_types.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {agency_types.length < perPage
                ? agency_types?.length + perPage * currentPage
...
export default TableSampleAgency_types
