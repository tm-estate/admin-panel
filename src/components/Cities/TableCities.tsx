...
const TableSampleCities = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { cities, loading, count, notify: citiesNotify } = useAppSelector((state) => state.cities)
  ...
  useEffect(() => {
    if (citiesNotify.showNotification) {
      notify(citiesNotify.typeNotification, citiesNotify.textNotification)
    }
  }, [citiesNotify.showNotification])
...
export default TableSampleCities
