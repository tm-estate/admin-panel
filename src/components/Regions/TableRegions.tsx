...
const TableSampleRegions = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (regionsNotify.showNotification) {
      notify(regionsNotify.typeNotification, regionsNotify.textNotification)
    }
  }, [regionsNotify.showNotification])
          ...
          <tbody>
            {regions &&
              ...
              regions.map((item: any) => (
                <tr key={item._id} onClick={() => router.push(`/regions/${item._id}`)}>
                  ...
                  <td data-label="cities">
                    {dataFormatter.citiesManyListFormatter(item.cities, 'titleRu').join(', ')}
                  </td>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {regions.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {regions.length < perPage
                ? regions?.length + perPage * currentPage
...
export default TableSampleRegions
