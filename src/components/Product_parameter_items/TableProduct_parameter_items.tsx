...
const TableSampleProduct_parameter_items = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (product_parameter_itemsNotify.showNotification) {
      notify(
        product_parameter_itemsNotify.typeNotification,
        product_parameter_itemsNotify.textNotification
      )
    }
  }, [product_parameter_itemsNotify.showNotification])
    ...
    <>
      ...
      <div className="relative overflow-x-auto">
        <table>
          ...
          <tbody>
            {product_parameter_items &&
              Array.isArray(product_parameter_items) &&
              product_parameter_items.map((item: any) => (
                <tr
                  key={item.id}
                  onClick={() => router.push(`/product_parameter_items/${item.id}`)}
                >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <div className="flex items-center mr-4">
            <p className={'text-[#8491A0]'}>
              Displaying {product_parameter_items.length === 0 ? '0' : currentPage * perPage + 1} to{' '}
              {product_parameter_items.length < perPage
                ? product_parameter_items?.length + perPage * currentPage
...
export default TableSampleProduct_parameter_items
