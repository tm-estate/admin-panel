...
const Dashboard = () => {
  ...
  async function loadData() {
    ...
    const responseDeal_types = await axios.get(`/dealTypes/count`)
    const responseProperty_types = await axios.get(`/propertyTypes/count`)
    Promise.all([responseUsers, responseDeal_types, responseProperty_types])
      .then((res) => res.map((el) => el.data.data))
      .then((data) => data.forEach((el, i) => fns[i](el)))
  }
            ...
            <div className="rounded dark:bg-gray-900/70 bg-white border border-pavitra-400 p-6">
              <div className="flex justify-between align-center">
                <div>
                  ...
                  <div className="text-3xl leading-tight font-semibold">{deal_types}</div>
                </div>
                <div>
                  <BaseIcon
                    className="text-blue-500"
            ...
            <div className="rounded dark:bg-gray-900/70 bg-white border border-pavitra-400 p-6">
              <div className="flex justify-between align-center">
                <div>
                  ...
                  <div className="text-3xl leading-tight font-semibold">{property_types}</div>
                </div>
                <div>
                  <BaseIcon
                    className="text-blue-500"
